...
pub async fn open_wallet(filename: String, secret: String, state: State<'_, AppState>) -> Result<String, ErrorResponse> {
    ...
    if let Some(wrpc_client) = wallet.try_wrpc_client().as_ref() {
        ...
        return Err(ErrorResponse { error: "No wRPC client available.".to_string() });
    }
    ...
    if let Some(prv_key_data_id) = key_data_id {
        store.as_prv_key_data_store()?.load_key_data(&wallet_secret, &prv_key_data_id).await.map_err(|e| {
            if e.to_string().to_lowercase().contains("decrypt") {
                ErrorResponse { error: "Incorrect password provided".to_string() }
            } else {
                ErrorResponse { error: format!("Failed to load private key data: {}", e) }
            }
        })?;
    } else {
    ...
    if let Some(account_id) = account_id {
        ...
        let account = wallet.get_account_by_id(&account_id, &guard).await?.ok_or_else(|| {
            ErrorResponse { error: format!("Account with ID {:?} not found", account_id) }
        })?;
    ...
    account.start().await.map_err(|e| {
        ErrorResponse { error: format!("Failed to start account: {}. Ensure node is reachable.", e) }
    })?;
